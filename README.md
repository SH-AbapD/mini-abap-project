...
사내에서 발생하는 실제 요청 업무(증명서 발급 · 비품 · 근무 변경 · 교육 신청 등)를 단일 워크플로우로 처리하는 시스템을 목표로, 구현 전 역할별 요구사항과 데이터 모델을 먼저 정의함.

### 역할별 요구사항

| 역할 | 요구사항 |
| --- | --- |
| 직원 | 요청 등록 / 본인 요청 조회 / 수정 / 취소 |
| 관리자 | 전체·상태별 조회 / 승인 / 반려 / 반려 사유 입력 |
| 공통 | 요청 유형 관리, 상태값 자동 관리, 날짜·필수값 검증, 승인 완료 건 수정 제한, 권한 분리 |

### 데이터 모델 설계

위 요구사항을 **마스터 4종 + 트랜잭션 1종**, 총 5개 테이블로 분해함.

| 구분 | 테이블 | 역할 |
| --- | --- | --- |
| 마스터 | `ZTR_DEPARTMENT` | 부서 |
| 마스터 | `ZTR_POSITION` | 직급 |
| 마스터 | `ZTR_EMPLOYEE` | 직원 (부서·직급 연결) |
| 마스터 | `ZTR_REQ_TYPE` | 요청 유형 |
| 트랜잭션 | `ZTR_REQ_HDR` | 요청 (승인 워크플로우) |

- 업무 의미가 있는 필드는 Data Element · Domain을 직접 정의해 재사용성·일관성 확보, 감사 필드는 SAP 표준 타입 활용
- 모든 테이블에 감사 필드(`CreatedAt/By`, `LastChangedAt/By`)를 공통 적용
...
승인·반려는 관리자만, 취소는 신청자 본인만 수행할 수 있도록 권한을 나눴습니다.
결재는 관리자 권한, 철회는 신청자 권한이라는 실제 결재 프로세스의 역할 분담을
시스템에 반영한 것입니다. 나아가 신청자용·관리자용 서비스를 별도로 분리해,
신청자는 본인 요청 조회·생성·취소만, 관리자는 전체 조회·승인·반려까지 다루도록
노출 범위 자체를 역할별로 나눴습니다.
...
개발 중 마주친 주요 이슈에 대한 원인 분석과 해결 과정입니다.

-> **[트러블슈팅 상세 보기](./TROUBLESHOOTING.md)**
